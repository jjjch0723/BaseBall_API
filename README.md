...
- **기능**: 매일 오전 10시에 스케줄링된 배치 프로그램을 통해 JSON 파일을 파싱하고, 데이터를 PostgreSQL 데이터베이스에 마이그레이션합니다.
- **데이터 처리**:
  - 최신 경기 일정, 경기 결과, 경기 예측 결과를 임시 테이블에 저장

### AWS EC2

- **기능**: API 서버와 PostgreSQL 데이터베이스를 호스팅
- **서비스**:
  - API 엔드포인트를 통해 최신 경기 일정과 결과 제공

### 예측 결과 처리

- **Python 스크립트 사용**: OpenAI API를 통해 경기 결과 예측 및 JSON 파일로 저장
- **Java 프로그램 역할**: 이 JSON 파일을 파싱하여 데이터베이스에 저장
