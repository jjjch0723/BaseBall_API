...
2. [Baseball API 아키텍처 구조](#21-baseball-api-아키텍처-구조)  
    ...
    2.3. [테이블 네이밍 룰](#23-테이블-네이밍-룰)  
    2.4. [데이터베이스 및 테이블 목록](#24-데이터베이스-및-테이블-목록)  
    2.5. [테이블 설계서](#25-테이블-설계서)
3. [상세 설명](#31-상세-설명)  
...
또한 실제로 web프로젝트와 DB가 통합되어있기에 이를 양해부탁드립니다. 아래는 web프로젝트의 깃 주소입니다.<br>
[프로젝트 링크](https://github.com/jjjch0723/ProjectForTheHime)
...
* 현재 `로스앤젤레스 에인절스`팀의 경기 예상 점수를 파싱 오류가 발생하고있습니다. 예상 점수를 0점으로 표시하고있습니다.(디버깅을 해보았으나 특정 한 팀에서만 발생하는 버그입니다. 원인불명......)
* `로스앤젤레스 에인절스`팀의 예상점수는 `0`점으로 처리 하고있습니다. 양해부탁드립니다.
* 서버에 서비스한 batch프로그램이 2달간 돌아가지 않았습니다. 수동으로 API의 엔드포인트를 요청해야 batch작업이 시작되어 서버의 문제라고 생각했으나, yml의 설정중 batch작업이 자동실행이 되지않도록 false를 입력해놓았습니다. 
